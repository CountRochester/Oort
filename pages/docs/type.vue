...
export default {
  ...
  methods: {
    ...
    async initialize () {
      try {
        this.busy = true
        if (!this.storage.types.items.length) {
          await this.storage.types.updateAll()
        }
        this.busy = false
      } catch (err) {
        consola.error(err)
    ...
    editItem (item) {
      ...
      this.editedItem = this.editedIndex >= 0
        ? this.storage.types.indexed[item.id].clone()
        : new Type()
    ...
    async deleteItem (item) {
      ...
      `
      this.busy = true
      confirm(`Вы уверены, что хотите удалить тип документа ${item.name}?`) && await gQLRequestMessage(this, query)
      if (this.$refs.form) {
