...
import editOrganisation from '@/components/edit-organisation'
...
export default {
  ...
  methods: {
    ...
    async initialize () {
      try {
        this.busy = true
        await this.storage.organisations.updateAll()
        this.busy = false
      } catch (err) {
        consola.error(err)
      }
    ...
    editItem (item) {
      ...
      this.editedItem = this.editedIndex >= 0
        ? this.storage.organisations.indexed[item.id].clone()
        : new Organisation()
    ...
    async deleteItem (item) {
      ...
      `
      this.busy = true
      confirm(`Вы уверены, что хотите удалить организацию ${item.orgName}?`) && await gQLRequestMessage(this, query)
      if (this.$refs.form) {
